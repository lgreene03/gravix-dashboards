...
func TestBoundaryMapHasExpectedCapabilityCount(t *testing.T) {
	...
	// 34 total: 31 core, 3 ee. Expanded from 18 by SD-002, then by Phase 8: the
	...
	// correctness suite (GRVX-810) and the SLO engine (GRVX-811).
	if got, want := len(m.Capabilities), 34; got != want {
		t.Errorf("capabilities = %d, want %d", got, want)
	}
	if got, want := len(m.CoreCapabilities()), 31; got != want {
