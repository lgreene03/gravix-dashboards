...
**There is no Gravix gem.** This recipe uses Ruby's standard-library `Net::HTTP` against the
documented REST contract, and everything below is what an `ApplicationController` `around_action`
would do.

Two things the other recipes get for free, you have to do yourself here:

- **Translate the route.** Nothing sanitizes the path, so send `/users/{id}`, never `/users/1234`.
  Sending a raw id is what makes dimension cardinality unbounded, and Gravix will reject a template
  containing a four-or-more-digit segment outright.
- **Generate a UUID version 7.** `SecureRandom.uuid` returns a **v4**, which is rejected with
  `event_id must be UUIDv7 (got v4)`. The recipe includes a `uuid_v7` helper for this reason.

Because there is no SDK there is also no batching: each fact is one `POST` to `/api/v1/facts`.

## The code

`examples/recipes/rails/client.rb` — this block and that file are byte-identical, and a test fails if they drift.
...
```bash
cd examples/recipes/rails
GRAVIX_ENDPOINT=http://localhost:8090 GRAVIX_API_KEY=$(cat ../../../data/api_key.txt) ruby client.rb
```

Prints `gravix responded 201`.
